...
// // Its linear because you dont know what the length is or it can change. 
...
function squareEveryDigit(number) {
    let separatedNumber = number.toString().split('')
      ...
      let squaredNumber = []
      let splitnumbers = []
      for (i = 0; i < separatedNumber.length; i++) {
        squaredNumber = Math.pow(separatedNumber[i],2);
        // console.log(squaredNumber);
        splitnumbers.push(squaredNumber)
        
      
      
      }
      
        let answer = splitnumbers.join('')
        console.log(answer)
